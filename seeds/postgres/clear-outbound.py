...
def main():
    ...
    cur.execute("SELECT COUNT(*) FROM stock_outbound")
    ...
    cur = conn.cursor()
    cur.execute("DELETE FROM stock_outbound_item")
    deleted_items = cur.rowcount
    cur.execute("DELETE FROM stock_outbound")
